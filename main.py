...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # ── RAG: build / load ChromaDB knowledge base ──────────────────
        ...
        log.info("RAG engine: %s (is_ready=%s)", _rag.get_stats(), _rag.is_ready)
        ...
        # Store on app.state so endpoints always read the live objects
        # regardless of uvicorn reload process boundaries
        app.state.rag      = _rag
        app.state.pipeline = _pipeline

        log.info("RiskTrace Agentic Pipeline ready  [mode: %s]", _db_mode)
    except Exception as e:
...
def rag_status():
    ...
    if rag is not None:
        ...
        return stats
    return {"ready": False, "message": "RAG not initialised"}
