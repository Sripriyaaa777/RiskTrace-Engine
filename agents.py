...
Six agents implementing the closed-loop agentic pipeline:
  ...
  Decision Agent      — Groq LLaMA 3.3 70B powered LLM explanation + recommendations
...
# ── RAG dependencies (graceful fallback if not installed) ──────────────────────
try:
    import chromadb
    ...
    logging.getLogger(__name__).warning(
        ...
        "RAGRetriever will fall back to keyword matching. "
...
class GraphReasoningAgent:
    ...
    def counterfactual_resolve(self, issue_id: str, resolve_as: str = "Done") -> dict:
        """
        Counterfactual reasoning: hypothetically resolve `issue_id` and
        rerun risk propagation on a cloned in-memory graph.
        No writes are made to the database.
        Returns before/after risk diff + full graph state for visual rendering.
        """
        nodes = self.perception.fetch_all_nodes_including_done()
        edges = self.perception.fetch_all_edges()
        if not any(n.issue_id == issue_id for n in nodes):
            raise ValueError(f"Issue {issue_id} not found")

        # BEFORE: propagation on real graph
        before_results = run_propagation(nodes, edges, self.config)

        # Clone & patch the target node to Done
        cloned_nodes = []
        for n in nodes:
            if n.issue_id == issue_id:
                cloned_nodes.append(IssueNode(
                    issue_id=n.issue_id, project=n.project, summary=n.summary,
                    status=resolve_as, priority=n.priority, assignee=n.assignee,
                    due_date=n.due_date, updated=n.updated,
                    delay_days=0.0, is_delayed=False,
                ))
            else:
                cloned_nodes.append(n)

        # AFTER: propagation on patched clone
        after_results = run_propagation(cloned_nodes, edges, self.config)

        # Build diff
        all_ids = (set(before_results.keys()) | set(after_results.keys())) - {issue_id}
        diff = []
        ...
        for nid in all_ids:
            ...
            if b_level == "High": high_before += 1
            if a_level == "High": high_after  += 1

            if delta < -0.01:
                improved += 1
                node_obj = next((n for n in nodes if n.issue_id == nid), None)
                if node_obj and node_obj.delay_days:
                    total_delay_saved += abs(delta) * max(node_obj.delay_days, 0.0)
            elif delta > 0.01:
                worsened += 1
...
class PlanningAgent:
    ...
    def create_mitigation_plan(
        ...
        candidates = {
            iid: r
            for iid, r in risk_results.items()
            if r.is_origin or (node_map.get(iid) and node_map[iid].is_at_risk())
        }

        for issue_id, result in candidates.items():
            ...
            # Mitigation should prioritize origins that can actually reduce
            # cascade risk, not isolated blocked issues with no dependents.
            impact_multiplier = downstream_count if downstream_count > 0 else 0.05
            priority_score = result.risk_score * priority_rank * impact_multiplier
...
# ─────────────────────────────────────────────
# RAGRetriever — semantic vector-based retrieval
# ─────────────────────────────────────────────

class RAGRetriever:
    """
    Semantic retriever that indexes IssueNode documents in ChromaDB and
    retrieves the top-k most relevant issues for a natural-language query
    using cosine similarity over sentence-transformer embeddings.

    Each issue is embedded as a rich document string:
        "<issue_id>: <summary>. Status: <status>. Priority: <priority>.
         Delay: <N> days overdue. Risk score: <score>."

    This allows the retriever to match semantic intent, not just keywords:
        - "tasks behind schedule" → matches issues described as "overdue" or "blocked"
        - "critical blockers" → matches high-priority Blocked issues
        - "root causes" → surfaces origin nodes with high risk propagation

    Falls back to the original keyword-matching logic when chromadb or
    sentence-transformers is not installed (import guard at module top).

    Lifecycle (called by AgentPipeline):
        rag.build_index(nodes, risk_results)   # called once at startup
        rag.retrieve(query, k)                 # called per user query
        rag.upsert_issue(node, risk_result)    # called by MonitoringAgent on state change
    """

    _MODEL_NAME = "all-MiniLM-L6-v2"
    _COLLECTION = "risktrace_issues"

    def __init__(self) -> None:
        self._ready = False
        if not _RAG_AVAILABLE:
            log.warning("RAGRetriever: dependencies missing, keyword fallback active.")
            return
        try:
            allow_download = os.getenv("RISKTRACE_ALLOW_MODEL_DOWNLOAD", "").strip().lower() in {
                "1", "true", "yes", "y",
            }
            ...
            # In-memory Chroma — no disk persistence needed; index rebuilt on each startup
            ...
            self._ready = True
            log.info("RAGRetriever initialised with model '%s'", self._MODEL_NAME)
        except Exception as exc:  # pragma: no cover
            log.warning(
                "RAGRetriever semantic initialisation failed (%s); keyword fallback active. "
                "Set RISKTRACE_ALLOW_MODEL_DOWNLOAD=true once to download the embedding model.",
                exc,
            )
    ...
    @staticmethod
    def _build_doc(node: "IssueNode", risk_result: Optional["RiskResult"] = None) -> str:
        """
        Compose the rich text document that gets embedded for this issue.
        Risk signals are repeated deliberately — repetition increases their
        weight in the embedding, making at-risk issues rank higher for
        queries about delays, blockers, and schedule problems.
        """
        delay_days = node.delay_days or 0
        is_overdue = delay_days > 0

        # Verbose delay phrase — repeated to boost embedding weight
        if is_overdue:
            delay_phrase = (
                f"This issue is overdue and behind schedule by {delay_days:.0f} days. "
                f"It is delayed and late. "
            )
        else:
            delay_phrase = "This issue is on schedule and not delayed. "

        # Verbose blocked phrase
        if node.status == "Blocked":
            blocked_phrase = "This issue is blocked and cannot proceed. It is a blocker. "
        else:
            blocked_phrase = ""

        risk_str = (
            f"Risk score: {risk_result.risk_score:.2f} ({risk_result.risk_level} risk). "
            if risk_result else ""
        )
        origin_str = (
            "This is a root cause origin driving cascading risk. "
            if (risk_result and risk_result.is_origin) else ""
        )

        return (
            f"{node.issue_id}: {node.summary}. "
    ...
    @staticmethod
    def _stable_chroma_id(issue_id: str) -> str:
        """
        ChromaDB requires string IDs that are unique and URL-safe.
        We deterministically derive one from the issue_id.
        """
    ...
    def _keyword_fallback(
        ...
        """
        Deterministic fallback used by every low-level retrieve() caller when
        semantic RAG is unavailable or fails. It scores issue IDs, summaries,
        status, priority, and common risk synonyms without requiring ChromaDB.
        """
        query_terms = [term for term in re.findall(r"[a-z0-9-]+", query.lower()) if len(term) > 1]
        if not query_terms:
            return list(node_map.values())[:k]

        synonym_groups = {
            "behind": {"behind", "schedule", "overdue", "late", "delayed", "delay", "missed", "deadline"},
            "schedule": {"behind", "schedule", "overdue", "late", "delayed", "delay", "missed", "deadline"},
            "overdue": {"behind", "schedule", "overdue", "late", "delayed", "delay", "missed", "deadline"},
            "late": {"behind", "schedule", "overdue", "late", "delayed", "delay", "missed", "deadline"},
            "blocked": {"block", "blocked", "blocking", "blocker", "stuck", "stalled", "cannot", "proceed"},
            "block": {"block", "blocked", "blocking", "blocker", "stuck", "stalled", "cannot", "proceed"},
            "critical": {"critical", "urgent", "high", "blocker"},
            "low": {"low", "routine", "minor", "normal"},
            "routine": {"low", "routine", "minor", "normal"},
        }
        expanded_terms = set(query_terms)
        for term in query_terms:
            expanded_terms.update(synonym_groups.get(term, set()))

        is_risk_query = bool(expanded_terms & {
            "behind", "schedule", "overdue", "late", "delayed", "delay",
            "missed", "deadline", "block", "blocked", "blocking", "critical",
            "urgent", "risk", "stuck", "stalled",
        })
        wants_low_risk = bool(expanded_terms & {"low", "routine", "minor", "normal"})

        scored: list[tuple[float, str, "IssueNode"]] = []
        for node in node_map.values():
            searchable = " ".join(
                part for part in [
                    ...
                    node.status,
                    node.priority,
                    "delayed overdue late behind schedule" if node.is_delayed else "on schedule routine",
                    "blocked blocker cannot proceed" if node.status == "Blocked" else "",
                ]
                if part
            ).lower()

            text_score = sum(1.0 for term in expanded_terms if term in searchable)
            delay_days = node.delay_days or 0.0
            risk_boost = 0.0
            if is_risk_query:
                risk_boost += min(delay_days / 30.0, 1.0)
                risk_boost += 1.0 if node.status == "Blocked" else 0.0
                risk_boost += {"Critical": 0.4, "High": 0.25, "Medium": 0.1}.get(node.priority, 0.0)
            if wants_low_risk:
                risk_boost += 1.0 if node.priority == "Low" else 0.0
                risk_boost += 0.5 if not node.is_at_risk() else -0.5

            total = text_score + risk_boost
            if total > 0:
                scored.append((total, node.issue_id, node))

        scored.sort(key=lambda item: (item[0], item[1]), reverse=True)
        return [node for _, _, node in scored[:k]]
    ...
    def build_index(
        ...
        """
        (Re)build the vector index from the full node list.
        Typically called once at AgentPipeline startup after the first
        global_risk_analysis() completes.
        """
        if not self._ready:
            return

        docs  = [self._build_doc(n, risk_results.get(n.issue_id)) for n in nodes]
        ids   = [self._stable_chroma_id(n.issue_id) for n in nodes]
        metas = [{"issue_id": n.issue_id} for n in nodes]

        # Embed in one batched call — much faster than per-issue calls
        embeddings = self._model.encode(docs, batch_size=64, show_progress_bar=False).tolist()
        ...
        log.info("RAGRetriever: indexed %d issues", len(nodes))
    ...
    def upsert_issue(
        ...
        """
        Insert or update a single issue in the index.
        Called by AgentPipeline whenever MonitoringAgent detects a state change,
        keeping retrieval results current without a full re-index.
        """
        if not self._ready:
            return
        doc = self._build_doc(node, risk_result)
        embedding = self._model.encode([doc], show_progress_bar=False).tolist()
        self._collection.upsert(
            ids=[self._stable_chroma_id(node.issue_id)],
            documents=[doc],
            embeddings=embedding,
            metadatas=[{"issue_id": node.issue_id}],
        )
        log.debug("RAGRetriever: upserted %s", node.issue_id)
    ...
    def retrieve(
        ...
        """
        Embed the query, fetch top-(k * fetch_multiplier) candidates by cosine
        similarity, then re-rank by a combined score:

            final_score = semantic_similarity * 0.7 + risk_boost * 0.3

        The risk_boost rewards at-risk nodes (delayed or blocked) so that a
        query like "tasks behind schedule" surfaces overdue issues even when
        the raw embedding similarity is close between candidates.

        Falls back to keyword matching when the vector index is unavailable,
        empty, or retrieval fails.
        """
        if not self._ready:
            return self._keyword_fallback(query, node_map, k)

        try:
            q_embedding = self._model.encode([query], show_progress_bar=False).tolist()
            # Fetch more candidates than needed so re-ranking has room to reorder
            n_fetch = min(k * fetch_multiplier, self._collection.count())
            if n_fetch <= 0:
                return self._keyword_fallback(query, node_map, k)
            ...
            # Detect if query is risk-oriented — only boost risk for relevant queries
            risk_keywords = {
                "behind", "schedule", "overdue", "late", "delay", "delayed",
                "block", "blocked", "blocking", "critical", "urgent", "risk",
                "stuck", "stalled", "missed", "deadline", "overrun", "slow",
            }
            query_words = set(query.lower().split())
            is_risk_query = bool(query_words & risk_keywords)
            risk_weight = 0.3 if is_risk_query else 0.0
            sem_weight  = 1.0 - risk_weight
            ...
            for meta, dist in zip(results["metadatas"][0], results["distances"][0]):
                ...
                # Risk boost: only applied for risk-oriented queries
                delay_days = node.delay_days or 0
                delay_boost = min(delay_days / 30.0, 1.0) if delay_days > 0 else 0.0
                blocked_boost = 0.3 if node.status == "Blocked" else 0.0
                risk_boost = min(delay_boost + blocked_boost, 1.0)
                final_score = similarity * sem_weight + risk_boost * risk_weight
            ...
            log.warning("RAGRetriever.retrieve failed (%s) — keyword fallback active", exc)
            return self._keyword_fallback(query, node_map, k)
...
class AgentPipeline:
    ...
    def __init__(self, db, openai_api_key: str, poll_interval: int = 30):
        ...
        # ── Real RAG: semantic vector retriever ───────────────────────────────
        self.rag = RAGRetriever()
        self._build_rag_index()  # warm the index at startup

    def start_monitoring(self):
        # Patch the MonitoringAgent's poll cycle to upsert changed nodes into RAG
        _original_poll_cycle = self.monitoring._poll_cycle

        def _patched_poll_cycle():
            _original_poll_cycle()
            # After each poll, re-upsert any changed nodes so retrieval stays current
            try:
                nodes = self.perception.fetch_all_nodes()
                risk_results = self.reasoning.global_risk_analysis()
                for node in nodes:
                    self.rag.upsert_issue(node, risk_results.get(node.issue_id))
            except Exception as exc:  # pragma: no cover
                log.warning("RAG upsert after poll failed: %s", exc)

        self.monitoring._poll_cycle = _patched_poll_cycle
    ...
    def _build_rag_index(self) -> None:
        """Fetch all nodes, run global risk analysis, and warm the RAG index."""
        try:
            nodes        = self.perception.fetch_all_nodes()
            risk_results = self.reasoning.global_risk_analysis()
            self.rag.build_index(nodes, risk_results)
        except Exception as exc:  # pragma: no cover
            log.warning("RAG index build at startup failed: %s", exc)

    # ── STEP 3: Semantic RAG — vector-similarity context retrieval ────────────
    def get_relevant_context(
        ...
        """
        Returns the top-k IssueNodes most semantically relevant to the query.

        Uses ChromaDB + all-MiniLM-L6-v2 embeddings (cosine similarity) so
        synonyms, paraphrases, and context are handled correctly:
            "tasks behind schedule" -> matches "overdue", "blocked", "delayed"
            "root causes"           -> surfaces is_origin=True nodes
            "critical blockers"     -> finds high-priority Blocked issues

        Falls back to the original keyword-matching heuristic when:
          - chromadb / sentence-transformers are not installed, OR
          - the vector index is empty (e.g. startup failure), OR
          - the collection has too few items to query meaningfully.

        The RAG index is built at startup (_build_rag_index) and kept live
        by start_monitoring which calls rag.upsert_issue() after each
        MonitoringAgent poll cycle.
        """
        node_map = {n.issue_id: n for n in nodes}

        # -- Attempt semantic retrieval ----------------------------------------
        semantic_results = self.rag.retrieve(query, node_map, k=k)
        if semantic_results:
            log.debug(
                "get_relevant_context: semantic RAG returned %d nodes for query %r",
                len(semantic_results), query,
            )
            return semantic_results

        # -- Keyword fallback ---------------------------------------------------
        log.debug("get_relevant_context: falling back to keyword matching for query %r", query)
        return self.rag._keyword_fallback(query, node_map, k)
