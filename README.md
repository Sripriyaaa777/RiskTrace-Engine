...
| | Feature | Description |
|---|---|---|
| 🕸️ | **Temporal Dependency Graph** | Builds a directed risk graph from JIRA-style CSVs or Neo4j — 300 synthetic issues and a 1,500-issue real Hadoop sample included |
...
└── data/
    ...
    └── real_hadoop/       ← 1,500 real Apache Hadoop JIRA issues
```
...
| `RISKTRACE_ALLOW_MODEL_DOWNLOAD` | `false` | Set `true` once if you want `all-MiniLM-L6-v2` downloaded from Hugging Face; otherwise semantic RAG uses a cached model if present and falls back quickly to keyword mode offline |
...
| Data | Apache Hadoop JIRA export sample (1,500 real issues) |
...
**Real Hadoop** (`data/real_hadoop/`) — 1,500 real Apache Hadoop JIRA issues in the included sample. The dependency file contains 4,353 edges: 26 explicit Jira issue-link edges and 4,327 inferred soft-dependency edges from text/similarity augmentation. A reproducible 100-edge heuristic validation sample is stored in `validation/edge_gold_template.csv`; against that sample, the inferred-edge set reports 0.500 precision, 1.000 recall, and 0.667 F1 in `validation/dependency_inference_results.json`. These labels are documented heuristics, not manually adjudicated ground truth, so dependency quality is reported as a current limitation rather than a finished benchmark.

Run `python3 check_stats_consistency.py --dataset data/real_hadoop` before submission to confirm `stats.json` still matches the CSV row count.
