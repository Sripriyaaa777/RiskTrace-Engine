...
# Predictive model: graph/time features can be enriched with frozen transformer
# text embeddings, then a scikit-learn LogisticRegression classifier is trained
# through a DictVectorizer Pipeline and evaluated with StratifiedKFold. The
# transformer is used only as a feature encoder; it is not fine-tuned.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train a scikit-learn LogisticRegression Jira risk model with optional transformer text embeddings, StratifiedKFold CV, and temporal holdout")
